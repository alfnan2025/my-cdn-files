...
(function () {
    document.addEventListener("DOMContentLoaded", function () {
        let t = document.title.includes("404"),
            s = window.location.pathname.indexOf("/search") !== -1,
            e = document.getElementById("error-message"),
            r = document.querySelector(".product"),
            c = JSON.parse(localStorage.getItem("cart")) || [],
            f = JSON.parse(localStorage.getItem("favorites")) || [];

        if (t) {
            e.textContent = "لم يتم العثور على الرابط 😢";
            e.style.display = "block";
        }

        if (s) {
            e.style.display = r ? "none" : "block";
        }

        function uF() {
            document.getElementById("favorites-notification").innerText = f.length;
        }
